...
# def run_job(job_id, uid, topic, characters, song, video_type):
...
def run_job(job_id, uid, topic, characters, song, video_type):
    """
    Runs the full video generation process for the given job_id.
    """
    job_dir = OUTPUT_BASE / job_id
    job_dir.mkdir(parents=True, exist_ok=True)
    metadata = {"status": "started", "steps": []}
    ...
    for idx, dlg in enumerate(dialogues):
        ...
        cmd = [
            ...
            "--output_rvc_path", str(output_audio),
            "--tts_voice", "en-US-GuyNeural",
            "--pth_path", f"C:/Users/jonat_rdnm2v4/Downloads/Applio-3.2.8-bugfix2/"
                          f"Applio-3.2.8-bugfix/logs/{char.lower()}/{char.lower()}.pth",
            "--index_path", f"C:/Users/jonat_rdnm2v4/Downloads/Applio-3.2.8-bugfix2/"
                            f"Applio-3.2.8-bugfix/logs/{char.lower()}/{char.lower()}.index"
        ]
...
def assemble_video(dialogues, audio_data, song, output_path):
    ...
    for idx, dlg in enumerate(dialogues):
        ...
        section = bg_clip.subclip(current_start, current_start + audio_clip.duration)
        composite = CompositeVideoClip([section, img_clip]).set_audio(audio_clip)
